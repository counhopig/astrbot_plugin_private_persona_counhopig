...
### 新增
- 休息系统：Bot 模拟人类作息，按时间段自动调整 Prompt 中的语气和状态
  - 4 种状态：清醒 / 犯困（睡前 1h）/ 睡觉（入睡 ~ 起床）/ 刚醒（起床后 1h）
  - 配置项：`rest_enabled`（总开关）、`rest_sleep_hour`（入睡时间，默认 23）、`rest_wake_hour`（起床时间，默认 7）
  - 睡觉时段主动问候自动跳过，不半夜打扰用户
  - 深夜被消息吵醒 → 回复简短含糊；睡前犯困 → 语气困倦；刚醒 → 慵懒慢半拍
