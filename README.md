...
| **主动插嘴** | ⚠️ 定时检测孤独主动问候 | ✅ Active Engagement 4.0 |
...
| **情感系统** | 活力 / 心情 / 社交需求，随时间自然衰减（后台 cron 自动更新），互动后恢复 |
...
| **休息系统** | Bot 模拟人类作息，晚上犯困/睡觉/早上迷糊/白天清醒，语气随状态变化 |
| **私聊专属** | 本插件专为私聊场景设计，`ignore_group_chat` 默认开启，群聊不受影响 |
...
- **情感衰减定时任务** — Crontab 格式，默认每小时对所有用户执行一次后台自然衰减（留空则仅在对话时衰减）
...
### 休息系统
- **启用休息系统** — 总开关，开启后 Bot 根据时间模拟人类作息状态
- **睡觉时间** — Bot 开始睡觉的小时数（24h，默认 23）
- **起床时间** — Bot 起床的小时数（24h，默认 7）
- 四种状态：清醒 / 犯困（睡前 1h）/ 睡觉 / 刚醒（起床后 1h），Prompt 语气随状态自动变化。睡觉时段主动问候自动跳过。
...
8. **休息系统** — 通过时间段计算（无需 cron），在 Prompt 中注入不同的休息状态指引，让 Bot 的语气自然呈现困倦/迷糊/慵懒等人类作息特征。
